...
Hi. I'm a starter theme called `zoilorios`. I'm a theme meant for hacking so don't use me as a Parent Theme. Instead try turning me into the next, most awesome, WordPress theme out there. That's what I'm here for.
...
`zoilorios` requires the following dependencies:
...
To start using all the tools that come with `zoilorios`  you need to install the necessary Node.js and Composer dependencies :
...
`zoilorios` comes packed with CLI commands tailored for WordPress theme development :
...
4. Search for `zoilorios` (in uppercase) to capture constants and replace with: `Nombre_proyecto`.
